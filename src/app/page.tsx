...
function HomeContent() {
  ...
  // TODO: 실제로는 API에서 커플 연결 상태를 가져와야 함
  ...
  useEffect(() => {
    const fetchCoupleStatus = async () => {
      ...
      try {
        // TODO: 실제 API 호출로 변경
        // const response = await api.get('/api/v1/couple/status');
        // setCoupleConnected(response.isConnected);
          ...
          <div className="flex items-center justify-center min-h-[60vh]">
            ...
            >
              ...
              <p className="mb-8" style={{ color: 'var(--text-secondary)' }}>
                프로필에서 커플 초대 링크를 생성하고<br />
                특별한 사람과 추억을 공유해보세요
              </p>

              <button
                onClick={() => router.push("/profile")}
                className="w-full px-6 py-3 text-white font-medium rounded-xl transition-all inline-flex items-center justify-center gap-2"
